...
class _studentOppPgaeState extends State<studentOppPgae> {
  ...
  Future<void> _applyNow() async {
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to apply for the role of ${_selectedOpportunity!.role}?'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancel')),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF422F5D)),
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: _selectedOpportunity == null
            ? CustomBottomNavBar(currentIndex: _currentIndex, onTap: _onNavigationTap)
            : null,
        bottomSheet: _selectedOpportunity != null ? _buildActionBottomSheet() : null,
      ),
    );
  }
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  _buildDateInfo(Icons.event_available_outlined, 'Apply By', formatDate(opportunity.applicationDeadline?.toDate())),
                ],
              ),
              const SizedBox(height: 12),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isApplying
          ...
          : Text("Apply Now", style: GoogleFonts.lato(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
      ),
    );
  }
  ...
  Widget _buildStatusAndWithdrawView() {
    ...
    final canWithdraw = status.toLowerCase() == 'pending' || status.toLowerCase() == 'reviewed';
    ...
    return Row(
      children: [
        Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Your Status:', style: GoogleFonts.lato(color: Colors.grey.shade600, fontSize: 12)),
            const SizedBox(height: 4),
        ...
        Expanded(
          child: ElevatedButton(
            ...
            child: Text(canWithdraw ? 'Withdraw' : 'Finalized', style: GoogleFonts.lato(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
        ),
      ],
    );
  ...
  Widget _buildDetailSection({required String title, required Widget content}) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: GoogleFonts.lato(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF422F5D))),
          const SizedBox(height: 12),
          content,
        ],
      ),
  ...
  Widget _buildChipList(List<String> items) {
    return Wrap(
      ...
      children: items.map((item) => Chip(
        ...
        labelStyle: const TextStyle(color: Color(0xFF422F5D), fontWeight: FontWeight.w600),
      )).toList(),
    );
  }
      ...
      child: Wrap(
        ...
        children: [
          if (_selectedCity != null && _selectedCity!.isNotEmpty)
            Chip(label: Text('City: $_selectedCity'), onDeleted: () { setState(() => _selectedCity = null); _cityController.clear(); _fetchOpportunities(); }),
          if (_selectedLocationType != null)
            Chip(label: Text(_selectedLocationType!.capitalize()), onDeleted: () { setState(() => _selectedLocationType = null); _fetchOpportunities(); }),
          if (_isPaid != null)
            Chip(label: Text(_isPaid! ? 'Paid' : 'Unpaid'), backgroundColor: _isPaid! ? Colors.green.shade100 : Colors.orange.shade100, onDeleted: () { setState(() => _isPaid = null); _fetchOpportunities(); }),
          if (_selectedDuration != null)
            Chip(label: Text(_selectedDuration!), onDeleted: () { setState(() => _selectedDuration = null); _fetchOpportunities(); }),
        ],
      ),
    );
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setModalState) {
          Widget buildModalChips<T>({ required List<T> options, required T? selectedValue, required String Function(T) labelBuilder, required Function(T?) onSelected, }) {
            return Wrap(
              spacing: 8.0,
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                buildModalChips<String>(options: ['remote', 'in-person', 'hybrid'], selectedValue: _selectedLocationType, labelBuilder: (s) => s.capitalize(), onSelected: (value) => setModalState(() => _selectedLocationType = value)),
                const SizedBox(height: 24),
                Text('Payment', style: GoogleFonts.lato(fontSize: 16, fontWeight: FontWeight.w600)),
                const SizedBox(height: 12),
                buildModalChips<bool>(options: [true, false], selectedValue: _isPaid, labelBuilder: (p) => p ? 'Paid' : 'Unpaid', onSelected: (value) => setModalState(() => _isPaid = value)),
                const SizedBox(height: 24),
                Text('Duration', style: GoogleFonts.lato(fontSize: 16, fontWeight: FontWeight.w600)),
                const SizedBox(height: 12),
                buildModalChips<String>(options: ["less than a month", "1-3 months", "3-6 months", "more than 6 months"], selectedValue: _selectedDuration, labelBuilder: (d) => d, onSelected: (value) => setModalState(() => _selectedDuration = value)),
