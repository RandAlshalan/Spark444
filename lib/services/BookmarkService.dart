...
class BookmarkService {
  ...
  Stream<List<Opportunity>> getBookmarkedOpportunitiesStream({
    required String studentId,
  }) {
    return _bookmarksRef
        .where('studentId', isEqualTo: studentId)
        ...
        .asyncMap((snapshot) async {
          final opportunityIds = snapshot.docs
              .map((doc) => doc.data().opportunityId)
              .toList();
          ...
          for (var i = 0; i < opportunityIds.length; i += batchSize) {
            ...
            allOpportunities.addAll(
              opportunitiesSnapshot.docs.map((doc) => doc.data()),
            );
          }
